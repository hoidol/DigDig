...
public class LevelUpCanvas : CanvasUI<LevelUpCanvas>
{
    ...
    public override void OpenCanvas(Action closeCallback = null)
    {
        ...
        int i = 0;
        foreach (var panel in levelUpBonusPanels)
        {
    ...
    List<LevelUpBonusPanel> PickBonusPanels()
    {
        var pool = levelUpBonusPanels.ToList();
        var result = new List<LevelUpBonusPanel>();

        bool isLowHp = Player.Instance.curHp < LOW_HP_THRESHOLD;
        if (isLowHp && UnityEngine.Random.value < FULL_HEAL_APPEAR_CHANCE)
        {
            var fullHealPanel = pool.FirstOrDefault(p => p.levelUpBonusType == LevelUpBonusType.FullHeal);

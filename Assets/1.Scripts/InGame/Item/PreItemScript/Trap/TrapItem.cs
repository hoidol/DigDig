...
public class TrapItem : TriggerItem
{
    ...
    public override void OnTrigger()
    {
        ...
        Vector3 spawnPos = Character.Instance.transform.position + (Vector3)(dir * throwDistance);


        RaycastHit2D hit = Physics2D.Raycast(Character.Instance.transform.position, dir, throwDistance, LayerMask.GetMask("Hittable"));
        if (hit.collider != null && hit.collider.GetComponent<Stone>() != null)
            spawnPos = Character.Instance.transform.position + (Vector3)(dir * (hit.distance - 0.5f));

...
public class WreckingBallItem : TriggerCycleItem
{
    ...
    {
        if (ball != null) return;
        ball = Instantiate(ballPrefab, Character.Instance.transform.position, UnityEngine.Quaternion.identity);
        ball.Init(Character.Instance.statMgr.AttackPower, DAMAGE_RATE);
    }

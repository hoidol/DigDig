...
public class Pickaxe : Ally
{
    ...
    async UniTaskVoid MineLoop(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            else
            {
                ...
                damageData.damage = Character.Instance.statMgr.AttackPower;
    ...
    Stone FindNearestOre()
    {
        Collider2D[] cols = Physics2D.OverlapCircleAll(
            Character.Instance.transform.position, SEARCH_RADIUS, LayerMask.GetMask("Hittable"));
        ...
        foreach (var col in cols)
        {
            ...
            float dist = ((Vector2)ore.transform.position - (Vector2)Character.Instance.transform.position).sqrMagnitude;
